...
description: Treat Claude Pro tokens as a real budget — the user can get rate-limited for hours if quota runs out
type: feedback
---

Pro plan tokens are a real cost, not "free." the user can hit the Claude Pro rate limit and get locked out for hours.
...
**Why:** If the user runs out of Pro tokens, he's locked out for hours.
...
- When a session starts, if the user flags token conservation, be explicit about it
