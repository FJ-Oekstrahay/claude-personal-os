...
`/load-handoff` shows the 5 most recent top-level HANDOFF files (excluding archive/) sorted newest-first using `ls -t`. the user picks by number.
...
**Why:** the user wanted a fast "just save this" command without the full session-handoff overhead. The old load-handoff was showing all 17+ files including archived ones with no sort guarantee.

**How to apply:** Recommend `/snapshot` when the user wants to save context quickly. Don't suggest cross-app context access is possible.
