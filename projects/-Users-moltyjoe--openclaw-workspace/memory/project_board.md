...
### Infrastructure
- **Browser Automation Hardening** — Playwright smoke + functional tests passed. Pending: resolve `.mcp.json` vs `~/.claude.json` config conflict, add anti-bot flag, run bot detection test, the user seeds cookies, failure mode tests, end-to-end via MoltyJoe.
...
- **Jarvis Talk Mode** — Config deployed. Pending: the user installs TestFlight app, connects to localhost:18789, sets wake word.
- **Discord channel permission matrix** — Pending: channel list from the user.
...
- **Prior Auth Voice Agent** — Phase 0 plan written. Next: the user gets Pablo's NPI + UHC prior auth number, runs Phase 0 recon call.
...
### Career Strategy
- **Strategy deep-dive** — Two Opus analyses done (Gary + Claudo). Next: the user reads both, answers 6 questions in BRAINSTORM doc, identifies first pilot prospect.
