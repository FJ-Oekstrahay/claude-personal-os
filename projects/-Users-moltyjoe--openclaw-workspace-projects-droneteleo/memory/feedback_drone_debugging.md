...
---
When the user brings a drone issue and it's connected, always use the droneteleo software to assist. Don't just give generic advice — actually connect, read params, and use the tools.
...
- Check memory for that quad's build/config info before asking the user
- Read current FC params directly instead of guessing
- If port is busy (Errno 16), immediately tell the user: "Port is busy — click the **Disconnect button** in Betaflight Configurator first." Use "Disconnect button" not just "disconnect" — user might physically unplug the cable otherwise.
