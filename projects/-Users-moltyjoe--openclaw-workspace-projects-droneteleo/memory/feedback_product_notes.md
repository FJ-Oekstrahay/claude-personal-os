...
- **Motor direction check videos** — the user wants to make Droneteleo-branded tutorial videos for motor direction verification (dollar bill method, prop A/B identification). These should be recorded during real debugging sessions.
...
- **Build memory** — The system should know which quad is connected and pull up build notes, last config changes, and known issues automatically. Currently requires reading from the user's memory files manually.
