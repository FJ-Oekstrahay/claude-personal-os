...
**Do not use `launchctl unload/kickstart`** — the modern launchd domain API (`kickstart`, `bootout`, `bootstrap`) doesn't work reliably for this service. `launchctl load` is what the user uses.
