...
| Type | Key | Behavior |
|------|-----|----------|
| **Site Key (Always Passes)** | `1x<channel-id>0AA` | Widget succeeds, token validates |
| **Site Key (Always Blocks)** | `2x<channel-id>0AB` | Widget fails visibly |
| **Site Key (Force Challenge)** | `3x<channel-id>0FF` | Always shows interactive challenge |
| **Secret Key (Testing)** | `1x<channel-id>000000000000AA` | Validates test tokens |
