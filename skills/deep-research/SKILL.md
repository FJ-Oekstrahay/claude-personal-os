...
When Geoff wants a deep, multi-source, fact-checked research report on any topic.
...
Default: ~28 total agents (MAX_FETCH=5, MAX_VERIFY_CLAIMS=5).

Math: 1 scope + ~6 search + 5 fetch + (5 claims × 3 votes) verify + 1 synth = ~28.
...
If Geoff needs more depth on a question, run it twice on sub-questions rather than raising the cap.
