---
name: visualize
description: Turn a data or visualization request into a chart delivered in-thread. Load when Geoff asks for a chart, plot, graph, histogram, heatmap, dashboard, or any visualization. Also triggers on "show me the data", "can you visualize", or "plot X vs Y". Default surface is Claude Code CLI — render to PNG and attach via Discord reply.
---
...
2. **Interactive / exploratory** (hover tooltips, zoom, filter, sliders, large point clouds, anything you'd want to poke at) → build a self-contained HTML file (vanilla + a CDN lib like Chart.js / Plotly / D3, no build step).
   ...
   - To let Geoff actually interact: save the HTML to a tmp dir and tell him the path to open in a browser.
...
- This is a **public repo** — write generated charts/HTML to `/tmp/` or a scratch dir, not into the repo, unless Geoff asks to keep them. Never commit data with PII.
