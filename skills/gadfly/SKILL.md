...
3. Save the full review output to:
   `~/.openclaw/workspace/projects/droneteleo/reviews/YYYY-MM-DD-gadfly-[topic].md`
   replacing YYYY-MM-DD with the actual date and [topic] with the slug.
4. Confirm the file path to Geoff.

If the review contains **[OPUS RECOMMENDED]**, tell Geoff explicitly.

Sequencing reminder: Gadfly runs BEFORE CTO. If Geoff wants both, run `/gadfly` first, then pass Gadfly's findings to `/cto`.
