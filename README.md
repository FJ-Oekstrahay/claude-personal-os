...
Most of these started the same way. I wanted to fly a drone, and instead spent the first evening hand-editing PID values out of a table. I wanted to print something, and lost an hour to slicer settings before the print even started. My kid and I wanted to build a game together, and had to learn an engine's project structure before either of us touched anything fun. Financial and health decisions hide the same wall behind duller language: weeks of gathering and cross-checking numbers before anyone can actually decide anything. Family logistics can do it too — a signup form, a schedule, tracking who has what — turning something that's supposed to be fun into one more thing to manage, for a kid as much as a parent. The tooling below exists to take that part off a person.
...
**Family infrastructure.** A Cloudflare Worker serving a shared family board, email and SMS intake workers feeding a structured wiki, an event-registration flow with a one-time-passcode gate, and a macOS screen-time daemon that had to work around the system's own notification suppression to be reliable. The point of all of it is the same as everywhere else on this list: a signup or a schedule shouldn't be what turns something meant to be fun into a chore, for a kid or a parent.
