...
Droneteleo, the drone-tuning tool below, now splits along that line. A small daemon runs on whichever machine has drones connected and serves flight-controller CLI access over the network — Tailscale carries that traffic; the Claude-to-Claude messaging above is a separate layer for the two sessions to talk to each other. Here's what that looks like in practice: I point a session at a flight controller plugged into a different machine, hand it the day's blackbox logs (or have Droneteleo pull them directly from the controller's flash), and tell it to analyze the logs, confirm the rate profile and parameters I want are still set, and stage a fix so it's ready to flash the moment I reconnect — then I close the laptop and go do something else while it works. The write-to-flash itself still waits for me to press it at the bench; that's how I've shaped the workflow, not something the daemon enforces. The same session is also reachable from my phone with the laptop closed: I can check what it found or point it somewhere else, through the same dictated-keyword relay this repo already uses for Discord.
...
That same review-and-hardware loop is also how Droneteleo itself gets built with more than one flight controller or radio connected at once, not just how it gets operated remotely afterward:
...
Claude Code is the only agent harness I use, even for work that isn't running on an Anthropic model. The provider is fixed per process rather than switchable mid-session: a workflow script routes a task to `ccx <target>`, which launches a separate Claude Code process against GLM, Kimi, or DeepSeek's backend. Those spawned workers don't even get their own worktree, the way Claude-tier ones do, because they really are a different process. I opt into this per turn, and only for that turn — most of those targets are metered per token, so routing off Claude's own models is a cost decision I make on purpose each time, not a default.

This only got worth doing recently. OpenRouter has added sticky routing and prompt caching over the past several months, in my experience, and that's what keeps a metered call from re-paying full context cost on every turn — for the targets I reach through OpenRouter; the one flat-rate plan I pay for doesn't go through it. Before that, routing off Anthropic's models cost more than it saved for most tasks. Now it doesn't, for the metered ones.

---

## Projects this config runs

Most of these started the same way: I wanted to do the thing, and there was a wall of setup in front of it first. Drones don't fly right until someone hand-edits PID values out of a table; a first print doesn't finish without an hour lost to slicer settings first. Building a game with my kid meant learning an engine's project structure before either of us had touched anything fun. Financial and health decisions hide the same wall behind duller language — weeks of gathering and cross-checking numbers before anyone can actually decide. Family logistics can throw it up too: a signup, a schedule, keeping track of who has what, turning something that's supposed to be fun into one more thing to manage. The tooling below exists to take that part off a person.
