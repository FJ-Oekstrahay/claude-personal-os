...
**`hooks/`** — shell scripts that fire at specific lifecycle points. `protect-sensitive-files.sh` blocks writes to live config and credentials on every `Write`, `Edit`, or `Bash` call. `discord-notify.sh` streams Claude's session to Discord in real time: narrative text blocks (what Claude is about to do, in its own words) followed by a one-line tool call summary for every tool use. For Agent (subagent) calls, it posts the subagent type, description, and the full prompt (capped at ~1500 chars) — every dispatch is visible in Discord. Also posts to an alerts channel when approval is needed. The companion `discord-text-extract.py` handles the JSONL transcript reading.

A second tier of hooks enforces the operating protocols rather than just observing them — the part a prompt can ask for but can't guarantee. `batchc-stop-gate.py` blocks a session from ending when a multi-file batch shipped without an independent verifier pass (`/verify` or `/code-review`) or a written handoff. `resource-pressure.py` and `wave-counter.py` track context-window fill and tool-call burst rate, so batch dispatch sizes its own waves and throttles before hitting rate limits instead of after. `batchc-nudge.py` flags prompts that look like multi-step work and suggests the batch protocol. `audit-trail.py` writes a per-tool-call forensic log. See [`hooks/README.md`](hooks/README.md) for design details.

**`commands/`** — user-invoked slash commands. `review-sequence` runs adversarial reviewers in the correct order (gadfly before CTO, or the CTO's plan anchors everything). `batchc` dispatches parallel subagent work with wave sizing, pressure-aware throttling, and a hook-enforced verifier gate. `plan-session` proposes a batch wave plan up front and stops for approval before dispatching anything. `mmguns` is a research-to-integration loop: websearch for SOTA tools and methodologies, gap-analyze against the current project, produce a ranked 3-item brief, then dispatch. `session-handoff` writes a structured resumption document so the next session can pick up without re-reading the full transcript. See [`commands/README.md`](commands/README.md) for the full list.

**`workflows/`** — deterministic multi-agent orchestrations driven by the Workflow tool, where control flow (fan-out, loops, verification gates) is code rather than model judgment. `deep-research.js` decomposes a question into search angles, fans out parallel web searches, fetches and dedupes the top sources, adversarially verifies each claim with a multi-vote panel, and synthesizes a cited report — with a hard agent cap so a deep run stays inside rate limits.
...
The full library is ~370 playbooks across these categories:
