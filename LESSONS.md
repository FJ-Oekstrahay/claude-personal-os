...
Don't instruct reading the whole file. If a skill reads a secrets file to get one API key, the entire file — including all other tokens — ends up in context. Extract what you need: `jq '.path.to.key' file.json`.
