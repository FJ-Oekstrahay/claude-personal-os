...
class GuildMemberManager extends CachedManager {
   ...
   * guild.members.fetch('<channel-id>')
   *   .then(console.log)
   *   .catch(console.error);
   * @example
   ...
   * guild.members.fetch({ user: ['<channel-id>', '<channel-id>'], withPresences: true })
   *   .then(console.log)
   *   .catch(console.error);
   * @example
   * // Fetch by query
   ...
   * guild.members.prune({ days: 7, roles: ['<channel-id>'] })
   *    .then(pruned => console.log(`I just pruned ${pruned} people!`))
   *    .catch(console.error);
   */
   ...
   * guild.members.unban('<channel-id>')
   ...
   * guild.members.bulkBan(['<channel-id>'], { deleteMessageSeconds: 7 * 24 * 60 * 60 })
   *   .then(result => {
