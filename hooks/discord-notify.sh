...
#
# ============================ ROUTING POLICY ==================================
# Two Discord destinations, NEVER the same content to both:
#
...
#
#   LOG channel (per-project firehose mirror, resolved via discord-routing.json):
#     - tool play-by-play (this file's PostToolUse/PreToolUse summary lines)
#     - narrative text firehose (discord-text-extract.py)
#     - post-reply trailing narrative (routed here by stop-check, not to discussion)
#
# Invariant: nothing posts verbatim to BOTH channels. The discussion-channel
# mirror of reply text was removed 2026-07-03 (it produced the duplicate copies).
# All outbound posts go through discord_outbound.py for dedup + coalescing + audit.
# =============================================================================

CONF="$HOME/.claude/hooks/discord-webhook.conf"
ERROR_LOG="$HOME/.claude/hooks/state/hook-errors.log"
HELPER="$HOME/.claude/hooks/discord_outbound.py"

_log_error() {
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] discord-notify: $1" >> "$ERROR_LOG" 2>/dev/null
}

# 2026-07-16: instrumentation for prompts/discord-inflight-visibility-gap.md Phase 1.
# One line per resolve_log_webhook() miss so we can measure how often sessions have
# no route to Discord at all, before picking a Phase 2 fix. session_id/cwd let us see
# whether misses cluster on fresh terminal sessions vs unmapped channels.
MISS_LOG="$HOME/.claude/hooks/state/webhook-resolve-misses.log"
_log_webhook_miss() {
  local reason="$1" session_id="$2" chat_id="$3"
  printf '{"ts":"%s","session_id":"%s","cwd":"%s","chat_id":"%s","reason":"%s"}\n' \
    "$(date '+%Y-%m-%d %H:%M:%S')" "$session_id" "$HOOK_CWD" "$chat_id" "$reason" \
    >> "$MISS_LOG" 2>/dev/null
}
...
#   $5=session (optional, for audit)  $6=channel (optional)  $7=target log|discussion|alert
post_webhook() {
  local url="$1" payload="$2" label="$3" auth_header="$4" attempt=0 code retry
  local a_session="$5" a_channel="$6" a_target="${7:-alert}"
  while [ $attempt -lt 3 ]; do
    ...
    if [ "$code" -ge 200 ] && [ "$code" -lt 300 ] 2>/dev/null; then
      [ -n "$a_session" ] && python3 "$HELPER" audit "$a_session" "$label" "$a_channel" "$a_target" "sent" "$code" "${payload:0:60}" 2>/dev/null
      return 0
    fi
  ...
  [ -n "$a_session" ] && python3 "$HELPER" audit "$a_session" "$label" "$a_channel" "$a_target" "dropped" "$code" "${payload:0:60}" 2>/dev/null
...
resolve_log_webhook() {
  ...
  if [ -z "$chat_id" ]; then
    _log_webhook_miss "no_chat_id" "$session_id" ""
    echo ""
    return
  fi
...
" "$routing_file" "$chat_id" 2>/dev/null)
  ...
  if [ -n "$var_name" ]; then
    ...
    _log_webhook_miss "missing_conf_var:$var_name" "$session_id" "$chat_id"
    echo ""
    return
  fi
  _log_webhook_miss "unmapped_chat" "$session_id" "$chat_id"
  echo ""
}

# Option B (2026-07-18): live-interleave subagent narrative into the log channel.
# On a subagent (Agent-tool, in-process) tool call the hook input carries a non-empty
# agent_id/agent_type, but transcript_path still points at the MAIN transcript (verified
# via spike 2026-07-18) — the subagent's own narrative lives in a sidechain file we must
# locate ourselves: "<transcript_path without .jsonl>/subagents/agent-<agent_id>.jsonl".
# discord-text-extract reads it under a PER-AGENT cursor and posts each block with a
# ↳[type · model] badge. Called from BOTH Pre- and PostToolUse so narrative flushes just
# before its own tool line (true interleave, same as the main-session firehose).
# Fail-open: any missing piece → silent return 0, never blocks the tool call.
flush_subagent_narrative() {
  local session_id="$1" transcript_path="$2" log_webhook="$3" agent_id="$4" agent_type="$5"
  [ -n "$agent_id" ] || return 0
  [ -n "$log_webhook" ] || return 0
  [ -n "$transcript_path" ] || return 0
  local subfile="${transcript_path%.jsonl}/subagents/agent-${agent_id}.jsonl"
  [ -f "$subfile" ] || return 0
  python3 "$HOME/.claude/hooks/discord-text-extract.py" \
    "$session_id" "$subfile" "$log_webhook" "$agent_id" "$agent_type" 2>/dev/null &
}

INPUT=$(cat)

# Used by _log_webhook_miss() above — read once, all resolve_log_webhook() callers
# below run after this point.
HOOK_CWD=$(echo "$INPUT" | /usr/bin/jq -r '.cwd // "unknown"' 2>/dev/null)

# SubagentStop (Option B Phase 2, 2026-07-18): flush the subagent's TRAILING narrative —
# the text it writes after its LAST tool call, which the Phase 1 Pre/PostToolUse interleave
# structurally can't reach (nothing fires after that last tool call). The event carries
# agent_transcript_path (the sidechain file itself — verified via spike) plus agent_id /
# agent_type. We reuse the SAME extractor + PER-AGENT cursor as Phase 1, so only the blocks
# after Phase 1's last flush post (idempotent — no double-post), then drain the coalesce
# buffer. Handled HERE, before the tool-name/Stop dispatch below: SubagentStop has no
# tool_name but DOES carry transcript_path, so without this guard it would fall into the
# Stop branch and misfire discord-stop-check. Fail-open: any missing piece → exit 0.
HOOK_EVENT=$(echo "$INPUT" | /usr/bin/jq -r '.hook_event_name // empty' 2>/dev/null)
if [ "$HOOK_EVENT" = "SubagentStop" ]; then
  SS_SESSION_ID=$(echo "$INPUT" | /usr/bin/jq -r '.session_id // empty' 2>/dev/null)
  SS_AGENT_ID=$(echo "$INPUT" | /usr/bin/jq -r '.agent_id // empty' 2>/dev/null)
  SS_AGENT_TYPE=$(echo "$INPUT" | /usr/bin/jq -r '.agent_type // empty' 2>/dev/null)
  SS_ATP=$(echo "$INPUT" | /usr/bin/jq -r '.agent_transcript_path // empty' 2>/dev/null)
  SS_TP=$(echo "$INPUT" | /usr/bin/jq -r '.transcript_path // empty' 2>/dev/null)
  # Prefer the event's own sidechain path; fall back to constructing it from agent_id
  # (same construction Phase 1 uses) if the field is ever absent.
  if [ -z "$SS_ATP" ] && [ -n "$SS_TP" ] && [ -n "$SS_AGENT_ID" ]; then
    SS_ATP="${SS_TP%.jsonl}/subagents/agent-${SS_AGENT_ID}.jsonl"
  fi
  if [ -n "$SS_SESSION_ID" ] && [ -n "$SS_AGENT_ID" ] && [ -f "$SS_ATP" ]; then
    SS_WEBHOOK=$(resolve_log_webhook "$SS_SESSION_ID")
    if [ -n "$SS_WEBHOOK" ]; then
      # Phase 3 fix (2026-07-18, Architect review finding 1): no flushall here.
      # text-extract already force-drains its narrative via urgent=True (see
      # discord-text-extract.py) — and subagents share the SAME (session, channel)
      # coalesce buffer as the parent (subagent-mode channel resolution keys off
      # the parent session's routechatid/chatid, per that file's comments). A
      # flushall on every subagent completion under a Workflow fan-out therefore
      # drains the shared main-session buffer once per subagent, defeating the
      # coalescing that protects the webhook from 429s. The only thing left
      # unflushed here is a trailing PostToolUse tool-line for THIS buffer key,
      # which the next hook invocation (main session resuming, or another
      # subagent) will flush within COALESCE_MAX_AGE (~2s), or the parent's own
      # Stop-time flushall will drain regardless. Do not reintroduce a flush
      # here without re-solving the 2026-07-12 orphaned-key bug (see the Stop
      # path below) — that's why this was flushall, not flush, in the first
      # place.
      python3 "$HOME/.claude/hooks/discord-text-extract.py" \
        "$SS_SESSION_ID" "$SS_ATP" "$SS_WEBHOOK" "$SS_AGENT_ID" "$SS_AGENT_TYPE" 2>/dev/null &
    fi
  fi
  exit 0
fi
...
if [ -n "$TOOL_NAME" ] && [ "$HAS_RESPONSE" = "no" ]; then
  ...
  AGENT_ID=$(echo "$INPUT" | /usr/bin/jq -r '.agent_id // empty' 2>/dev/null)
  AGENT_TYPE=$(echo "$INPUT" | /usr/bin/jq -r '.agent_type // empty' 2>/dev/null)
  ...
  if [ -n "$SESSION_ID" ] && [ -n "$TRANSCRIPT_PATH" ]; then
    ...
    flush_subagent_narrative "$SESSION_ID" "$TRANSCRIPT_PATH" "$ACTIVE_LOG_WEBHOOK" "$AGENT_ID" "$AGENT_TYPE"
  fi
...
try:
    ...
    if tool == 'Skill':
        ...
        agent_name = ti.get('name', '') or ''
        # model param is an explicit override; absent means the agent inherits
        # the parent session's model.
        model = ti.get('model', '') or 'inherit'
        who = f'**{safe_backtick(agent_name, 40)}** ' if agent_name else ''
        print(f'🤖 **Agent dispatch:** {who}[{safe_backtick(subagent_type, 30)} · {safe_backtick(model, 20)}] \`{safe_backtick(desc, 80)}\`')
...
" 2>/dev/null)
  ...
  if [ -n "$PRE_MSG" ]; then
    ...
    if [ -z "$ACTIVE_LOG_WEBHOOK" ]; then
      ...
      # Coalesced + deduped + audited log-channel post (buffer flushes at ~2s / Stop).
      PRE_CHAT_ID=$(_resolve_chat_id "$SESSION_ID")
      python3 "$HELPER" buffer "$SESSION_ID" "${PRE_CHAT_ID:-log}" "$ACTIVE_LOG_WEBHOOK" "$PRE_MSG" 2>/dev/null &
    fi
  fi
  ...
  AGENT_ID=$(echo "$INPUT" | /usr/bin/jq -r '.agent_id // empty' 2>/dev/null)
  AGENT_TYPE=$(echo "$INPUT" | /usr/bin/jq -r '.agent_type // empty' 2>/dev/null)
  # Phase 3 fix (2026-07-18, Architect review finding 3): resolve exactly once,
  # unconditionally, so PreToolUse/SubagentStop's one-miss-per-event pattern
  # holds here too. Previously this was only set inside the guard below, then
  # re-resolved via `${POST_LOG_WEBHOOK:-$(resolve_log_webhook ...)}` further
  # down whenever the guard was skipped or resolved empty — logging a second
  # _log_webhook_miss per event and skewing the 1:1 miss-rate instrumentation
  # webhook-resolve-misses.log exists to measure.
  POST_LOG_WEBHOOK=$(resolve_log_webhook "$SESSION_ID")

  if [ -n "$SESSION_ID" ] && [ -n "$TRANSCRIPT_PATH" ]; then
    python3 "$HOME/.claude/hooks/discord-text-extract.py" \
      "$SESSION_ID" "$TRANSCRIPT_PATH" "$POST_LOG_WEBHOOK" &
    flush_subagent_narrative "$SESSION_ID" "$TRANSCRIPT_PATH" "$POST_LOG_WEBHOOK" "$AGENT_ID" "$AGENT_TYPE"
  fi
...
try:
    ...
    if tool == 'Write':
        ...
        model = ti.get('model', '') or 'inherit'
        who = f'**{safe_backtick(agent_name, 40)}** ' if agent_name else ''
        header = f'**Agent** → {who}\`{safe_backtick(subagent, 40)}\` [{safe_backtick(model, 20)}]'
        if desc:
...
" 2>/dev/null)
  ...
  # Tool-line attribution (Option B bonus): tag subagent tool lines so the log stream
  # shows they came from a subagent, matching the ↳[type · model] narrative badge.
  [ -n "$AGENT_TYPE" ] && [ -n "$MSG" ] && MSG="↳[$AGENT_TYPE] $MSG"

  WEBHOOK_URL="$POST_LOG_WEBHOOK"
  ...
  if [ -n "$SESSION_ID" ] && [ -n "$TRANSCRIPT_PATH" ]; then
    ...
    # text-extract advances the state file so stop-check doesn't double-post — which
    # only holds if text-extract FINISHES before stop-check starts. Both were previously
    # backgrounded concurrently; whichever won the state-file race decided whether the
    # trailing text landed in the log channel, the discussion channel, or both.
    # Sequenced inside one backgrounded group: deterministic handoff, still zero
    # latency added to Claude.
    STOP_LOG_WEBHOOK=$(resolve_log_webhook "$SESSION_ID")
    {
      if [ -n "$STOP_LOG_WEBHOOK" ]; then
        ...
        # Force-flush any coalesced log-channel buffer so trailing play-by-play
        # lands before the done-ping (avoids losing the turn's tail).
        # flushall, not flush: the old call passed _resolve_chat_id's key, but
        # text-extract buffers under the chat id parsed from the transcript's
        # <channel> tag. Those differ for thread sessions, so the flush looked up a
        # key that was never written — 129 of 131 orphaned .logbuf files (2026-07-12).
        # flushall globs by session and is agnostic to which key the producer used.
        python3 "$HELPER" flushall "$SESSION_ID" "$STOP_LOG_WEBHOOK" 2>/dev/null
      fi
      # Item 7 (2026-07-13): pass the already-resolved log webhook through as an
      # explicit arg instead of letting discord-stop-check.py re-derive it from
      # $STOP_LOG_WEBHOOK's conf var name via os.environ — that var is set by
      # `source "$CONF"` above (a bash local), never `export`ed, so the Python
      # child could never see it. That was the root cause of the 8 all-time
      # Stop-postreply-log drops logged as http=no-url.
...
" 2>/dev/null)
  ...
  if [ -f "$CHATID_FILE" ] && [ -n "$BOT_TOKEN" ]; then
    ...
    post_webhook "https://discord.com/api/v10/channels/$CHAT_ID/messages" \
      "$DIRECT_PAYLOAD" "Notification-botapi" "Authorization: Bot $BOT_TOKEN" \
      "$SESSION" "$CHAT_ID" "discussion" &
  fi
  ...
  if [ -n "$ACTIVE_LOG_WEBHOOK" ]; then
    ...
    post_webhook "$ACTIVE_LOG_WEBHOOK" "$LOG_PAYLOAD" "Notification-log" "" "$SESSION" "" "log" &
  fi
...
Session: \`${SESSION_SHORT}\`  CWD: \`${CWD_SHORT}\`"
    ...
    post_webhook "$ALERTS_WEBHOOK_URL" "$ALERTS_PAYLOAD" "Notification-alerts" "" "$SESSION" "" "alert" &
  fi
...
# Coalesced + deduped + audited log-channel play-by-play line.
POST_CHAT_ID=$(_resolve_chat_id "$SESSION_ID")
python3 "$HELPER" buffer "$SESSION_ID" "${POST_CHAT_ID:-log}" "$WEBHOOK_URL" "$MSG" 2>/dev/null &
