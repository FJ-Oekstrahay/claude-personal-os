...
| `pressure` | Manually override session pressure level (`normal`/`elevated`/`high`) in the state file. Used when the auto-tracker misreads context fill. All pressure-aware commands (batchc, mmguns, review-sequence) throttle accordingly until cleared. | Yes |
