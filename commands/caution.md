Geoff prefaced a task with **"caution"** (or invoked `/caution`). This is a standing signal: before doing the work, check whether any part of it could collide with other Claude Code sessions running on this machine, then split the work into **safe-now** vs **defer**. Do not guess glibly — walk this procedure.
...
3. **Classify each subtask** by the shared resources it would touch:
   ...
   **DEFER (do NOT edit while peers run):**
   ...
   - `session-handoff` tooling (command/skill/scripts) — peers run handoffs at wind-down; Geoff named this class explicitly.
...
5. **For each DEFERRED subtask:** write a fix/exec prompt to `<cwd>/prompts/<name>.md` following Geoff's prompt convention (model recommendation, run order, whether it can be orchestrated as a parallel workflow, constraints). Do NOT apply the change.
6. **Report back with all of:**
   ...
   - (d) the prompt path(s) written, so Geoff can re-run later.
