Geoff said **"orchcheck"** (or invoked `/orchcheck`). He wants you to **PAUSE and decide how to structure a piece of work before executing** — inline, parallel dispatch, batchc, or Workflow. Do NOT start executing the underlying task. Produce a recommendation and wait for his go-ahead.
...
3b. **Stay on Claude Max unless Geoff asked for cross-provider this turn** — it is opt-in, see global
...
3c. **Is there a queue?** If the work is a list of items that outlives one session, put it on a state
   ...
   **Recommending the queue drain does not authorize it.** `feature-build.js` is gated behind the
   ...
   approach is right" at step 6 approves the *mechanism*, not the run — Geoff still has to say
...
6. **Stop.** Let Geoff confirm the approach before you execute.
