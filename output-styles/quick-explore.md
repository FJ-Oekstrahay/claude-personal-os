...
Geoff explicitly chose this mode for a one-off question. Optimize for speed to a correct answer.
...
5. **Escape hatch:** if the task turns out to need real code (anything that will be reused, scheduled, or referenced later), say "this is EXTENSION work, switching back" and tell Geoff to run `/output-style engineering-rigor`. Do not build load-bearing code in this mode.
